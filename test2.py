...
# input_annons_score = len(annons_res[0])

# df.drop(df[df['Mask_score'] >= input_annons_score].index, inplace = True)
...
# output_dict['similar_ad'] = df['description.text'][most_similar_index]
# output_dict['mask_score'] = df['Mask_score'][most_similar_index]

# print(output_dict)


def calc_similarity3(input_annons, input_annons_score, data, employer_name, annons_text, mask_score):
...
def re_vectorize(modified_df, input_annons):

    # Load the vectorizer from the saved vectorized data
    with open('vectorized_data.pkl', 'rb') as f:
        vectorized_data = pickle.load(f)

    vectorizer = vectorized_data['vectorizer']

    # Preprocess and vectorize the modified dataframe
    preprocessed_texts = modified_df['description.text'].apply(preprocessor)
    ...
    ## Sorterar och hämtar mest liknande annons och företag
    most_similar_index = sim_scores.argsort()[-1]
    most_similar_other_column = modified_df['employer.name'][most_similar_index]
    sim_score = sim_scores[most_similar_index]
    ...
    output_dict['similarity_score'] = sim_score
    output_dict['employer'] = most_similar_other_column
    output_dict['similar_ad'] = modified_df['description.text'][most_similar_index]
    output_dict['mask_score'] = modified_df['Mask_score'][most_similar_index]
